...
for fichier in `ls $dossier_urls`
{
	...
	for line in `cat $dossier_urls/$fichier`
	{
		...
		#motif=$(egrep -i -1 "((coc(h)?l(é|e)?a(i)?r(e|s)*?)?( )?implant(e|s)*?( )?(coc(h)?l(é|e)?a(i)?r(e|s)*?)?)|(\b( |')IC\b|\b CI(^-)\b)" ../pages_dump/$compteur_tableau-$compteur_ligne.txt) ;
		#contexte_html=$(egrep -i -1 "((coc(h)?l(é|e)?a(i)?r(e|s)*?)?( )?implant(e|s)*?( )?(coc(h)?l(é|e)?a(i)?r(e|s)*?)?)|(\b( |')IC\b|\b CI(^-)\b)" ../pages_aspirees/$compteur_tableau-$compteur_ligne.html) ;
		...
		if [[ $? == 0 ]]
			...
			if [[ $encodagegrep == "utf-8" ]]
				...
				sed 's/^[\t| \s]*//;/^\(#\|*\|+\|o \)/d' ../pages_dump/brut$compteur_tableau-$compteur_ligne.txt | awk '!a[$0]'++ > ../pages_dump/$compteur_tableau-$compteur_ligne.txt ;
                                egrep -i -1 "((coc(h)?l(é|e)?a(i)?r(e|s)*?)?( )?implant(e|s)*?( )?(coc(h)?l(é|e)?a(i)?r(e|s)*?)?)|(\b( |')IC\b|\b CI(^-)\b)" ../pages_dump/$compteur_tableau-$compteur_ligne.txt > ../contextes/$compteur_tableau-$compteur_ligne.txt ;
				...
				#echo $contexte_html >>../contextes/$compteur_tableau-$compteur_ligne.html ;
				...
				if [[ $encodageiconv == "" ]]
					...
					encodageiconv=$(iconv -l | egrep -oi "\b$encodagefile\b") ;
						...
						then
							...
							cat ../pages_dump/$compteur_tableau-$compteur_ligne.txt >> ../pages_dump/globaldump-$compteur_tableau.txt ;
							cat ../contextes/$compteur_tableau-$compteur_ligne.txt >> ../contextes/globalcontextes-$compteur_tableau.txt ;
							#echo $motif >> ../contextes/$compteur_tableau-$compteur_ligne.txt ;
							#echo $contexte_html >>../contextes/$compteur_tableau-$compteur_ligne.html ;
							#perl ../minigrepmultilingue-v2.2-regexp/minigrepmultilingue.pl "utf-8" ../pages_dump/$compteur_tableau-$compteur_ligne.txt ../minigrepmultilingue-v2.2-regexp/motif-regexp.txt ;
							#mv ../minigrepmultilingue-v2.2-regexp/resultat-extraction.html ../contextes/$compteur_tableau-$compteur_ligne.html ;
						else
							...
							echo "<tr><td>Lien n&deg; $compteur_ligne</td><td><a href=\"$line\">URL n&deg;$compteur_ligne</a></td><td><a href=\"../pages_aspirees/$compteur_tableau-$compteur_ligne.html\">PA n&deg;$compteur_ligne</a></td><td> Non sp&eacute;cifi&eacute; </td><td> -- </td><td> -- </td><td> -- </td><td> -- </td><td> -- </td></tr>" >>$fichier_tableau ;
								...
								egrep -i -1 "((coc(h)?l(é|e)?a(i)?r(e|s)*?)?( )?implant(e|s)*?( )?(coc(h)?l(é|e)?a(i)?r(e|s)*?)?)|(\b( |')IC\b|\b CI(^-)\b)" ../pages_dump/$compteur_tableau-${compteur_ligne}_UTF8.txt > ../contextes/$compteur_tableau-${compteur_ligne}_UTF8.txt ;
								...
								#echo $motif >> ../contextes/$compteur_tableau-${compteur_ligne}_UTF8.txt ;
								#echo $contexte_html >>../contextes/$compteur_tableau-${compteur_ligne}_UTF8.html ;
